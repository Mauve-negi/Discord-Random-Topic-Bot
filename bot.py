...
async def on_ready():
    ...
    # 起動直後にテスト投稿
    print("🧪 起動確認：お題を1件投稿します")
    await post_daily_topic()
...
async def post_daily_topic():
    channel = bot.get_channel(TOPIC_CHANNEL_ID)
    if channel is None:
        print("⚠️ 指定チャンネルが見つかりません。IDを確認してください。")
        return
...
async def on_message(message):
    ...
    if message.channel.id == THEME_CHANNEL_ID:
        if TICKET_ROLE_NAME in [r.name for r in message.author.roles]:
